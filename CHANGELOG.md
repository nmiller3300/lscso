...
- **Historical Archives — Physical 3D Folder Interaction**: Adapted the public 3D-folder interaction into the real historical collection browser using LSCSO manila/kraft folders, real folder names, real record counts, layered paper depth, selection lift, and responsive phone/tablet/desktop presentations without duplicating archive data or navigation logic.
- **Open Records — Real Upload Progress UI**: Adapted the public file-upload presentation into the Records Custodian workflow with actual filename/size display, browser-reported upload progress, honest indeterminate fallback, completion/error states, and the existing protected Open Records upload endpoint rather than simulated progress.
- **Personnel Portal — Layered Notification Stack**: Adapted the public notification-stack depth treatment into the existing Portal attention system so real unread LSCSO notification records and pending action counts appear as a compact layered stack linked to the existing Notification & Action Center rather than creating a second inbox.
- **Governance — Cross-Platform Release Standard**: Added a permanent cross-platform QA standard covering small/large phones, tablet portrait/landscape, laptop/desktop, touch/coarse pointer, mouse/keyboard, WebKit/Safari, Chromium, safe areas, orientation changes, dynamic viewport behavior, and reduced-motion usability.
- **Governance — Responsive Static Audit**: Added a CI audit that rejects device-sniffing and mouse-only/touch-only interaction handlers and verifies the root viewport contract remains device-width, initial-scale 1, and safe-area capable.
...
- **UI Development — Cross-Platform Baseline**: Changed responsive development expectations from device-specific fixes to a whole-site release requirement: core functionality must remain available across phone, tablet, and desktop layouts, required actions cannot depend on hover, and touch/keyboard/pointer behavior must be designed together.
...
- **Historical Archives — Cross-Platform Folder Visibility**: Removed the desktop-only cutoff that caused the physical folder treatment to disappear on phone and narrow-tablet layouts; smaller viewports now receive a compact tactile folder presentation instead of silently reverting to a different interaction.
- **Historical Archives — Lamp Touch Target**: Expanded the lamp pull’s coarse-pointer hit area to a practical touch target while retaining the visually small brass pull, reducing missed taps/drags without changing the lamp artwork.
- **Personnel Portal — Notification Stack Safari Compatibility**: Replaced calculated stack-depth transforms with explicit card transforms so the layered notification treatment remains consistent in Safari/WebKit as well as Chromium.
