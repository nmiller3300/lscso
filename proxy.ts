...
export async function proxy(request: NextRequest) {
  ...
  const maintenanceRoute = pathname === "/maintenance" || pathname === "/portal/maintenance" || pathname === "/portal/maintenance-access";
  const sessionResponse = portal ? await updateSession(request) : NextResponse.next({ request });
  ...
  const active = states.find((item) => item.scope === scope)?.effective_mode === "maintenance";
  if (!active) return sessionResponse;
