...
begin
  ...
  if v_old.rank is distinct from p_rank and v_new_level>v_old_level then raise exception 'Promotions must be completed through Promotion Review'; end if;
  if v_actor_rank='Major' and (v_old_level>=app_private.rank_level('Major') or v_new_level>=app_private.rank_level('Major')) then raise exception 'Major may approve personnel changes below Major rank only'; end if;
  ...
  if v_old.rank is distinct from p_rank then
    v_event_type:=case when v_new_level<v_old_level then 'Demotion' else 'Rank Correction' end;
  ...
  if app_private.fivem_portal_integration_enabled() and (v_old.rank is distinct from p_rank or v_old.status is distinct from p_status) then
    ...
    order by fil.linked_at desc limit 1;
...
create or replace function public.v2_update_personnel_identity(p_profile_id uuid, p_rank text, p_status text, p_reason text default null::text)
returns public.personnel_profiles
language plpgsql security definer set search_path=''
as $$
declare
