...
begin
  ...
  if v_stage = 'Executive Command' then
    return query select
      null::uuid,
      'Sheriff / Undersheriff'::text,
      'Executive Command'::text,
      'Executive Command'::text,
      true;
    return;
  end if;
  ...
  if v_stage = 'Training Review' then
    ...
    order by
      case sa.authority_type when 'Training' then 0 when 'Command' then 1 when 'Primary' then 2 else 3 end,
      sa.starts_at desc
    limit 1;
  ...
  -- First preference: explicit direct supervisory authority over the member.
  select sa.supervisor_profile_id,
         trim(concat_ws(' ', sp.rank, sp.display_name))
    into v_reviewer, v_reviewer_label
  from public.supervisory_authorities sa
  ...
  where sa.ends_at is null
    ...
    and sa.supervisor_profile_id is distinct from p_exclude_reviewer
  order by
    case sa.authority_type when 'Primary' then 0 when 'Command' then 1 when 'Conflict Reassignment' then 2 when 'Temporary' then 3 else 4 end,
    sa.starts_at desc
  limit 1;

  -- Second preference: command authority over the member's primary organizational unit.
...
begin
  select * into route
  from app_private.resolve_personnel_request_route(
    new.requester_profile_id,
    new.request_type,
    new.requested_unit_id,
    null,
    null
  );
...
-- Replace the legacy self-only status notification trigger with routed notifications.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    values(
      ...
      request_ref || ' was submitted. Current routing: ' || coalesce(new.current_reviewer_label,new.routing_label,'Command review') || '.',
    ...
    if new.current_reviewer_profile_id is not null and new.current_reviewer_profile_id <> new.requester_profile_id then
      insert into public.notifications(recipient_profile_id,notification_type,title,message,href)
      values(
        new.current_reviewer_profile_id,
        'Personnel Request Review',
        'New ' || new.request_type || ' request',
        coalesce(requester_name,'Personnel') || ' submitted ' || request_ref || '. This request is assigned to you for ' || coalesce(new.routing_label,'review') || '.',
        '/portal/command/approvals#personnel-requests'
      );
      ...
      select p.id,
             'Personnel Request Review',
             'New ' || new.request_type || ' request',
             coalesce(requester_name,'Personnel') || ' submitted ' || request_ref || '. No assigned reviewer was available; Executive Command review is required.',
             '/portal/command/approvals#personnel-requests'
      from public.personnel_profiles p
      where p.rank in ('Sheriff','Undersheriff')
        and p.status in ('Active','Acting')
        and p.id <> new.requester_profile_id;
    end if;
  ...
  if new.status is distinct from old.status and new.status in ('Approved','Denied','Cancelled','Completed') then
    ...
    values(
      new.requester_profile_id,
      'Request Decision',
      new.request_type || ' request ' || lower(new.status),
      request_ref || ' is now ' || new.status || '.',
      '/portal/my-office#requests'
    );
    return new;
  end if;
  ...
  if new.routing_stage is distinct from old.routing_stage
    ...
    values(
      new.requester_profile_id,
      'Request Routing',
      new.request_type || ' request advanced',
      request_ref || ' is now with ' || coalesce(new.current_reviewer_label,new.routing_label,'Command') || '.',
      '/portal/my-office#requests'
    );
    ...
    if new.current_reviewer_profile_id is not null and new.current_reviewer_profile_id <> new.requester_profile_id then
      ...
      values(
        new.current_reviewer_profile_id,
        'Personnel Request Review',
        new.request_type || ' request assigned',
        request_ref || ' from ' || coalesce(requester_name,'Personnel') || ' is now assigned to you for ' || coalesce(new.routing_label,'review') || '.',
        '/portal/command/approvals#personnel-requests'
      );
    elsif new.routing_fallback then
      insert into public.notifications(recipient_profile_id,notification_type,title,message,href)
      select p.id,
             'Personnel Request Review',
             new.request_type || ' request requires Executive review',
             request_ref || ' from ' || coalesce(requester_name,'Personnel') || ' is now awaiting Executive Command action.',
             '/portal/command/approvals#personnel-requests'
      from public.personnel_profiles p
      where p.rank in ('Sheriff','Undersheriff')
        and p.status in ('Active','Acting')
...
begin
  ...
  if decision = 'Denied' then
    insert into public.personnel_request_route_events(
      ...
      request_record.id,'Denied',coalesce(request_record.routing_label,request_record.routing_stage,'Review'),request_record.current_reviewer_profile_id,request_record.current_reviewer_label,current_profile,actor_label,nullif(trim(review_notes),'')
    );
  ...
  insert into public.personnel_request_route_events(
    ...
    request_record.id,'Stage Approved',coalesce(request_record.routing_label,request_record.routing_stage,'Review'),request_record.current_reviewer_profile_id,request_record.current_reviewer_label,current_profile,actor_label,nullif(trim(review_notes),'')
  );
  ...
  elsif request_record.routing_stage in ('Training Review','Executive Command') then
    next_route := null;
  ...
  if next_route is not null then
    update public.personnel_requests
  ...
  insert into public.personnel_request_route_events(
    ...
    request_record.id,'Approved','Completed · Approved',current_profile,actor_label,nullif(trim(review_notes),'')
  );
...
-- Backfill currently open requests without re-notifying personnel.
do $$
declare
  request_row record;
  route record;
...
begin
  ...
  loop
    ...
    select p.display_name into requester_name
    from public.personnel_profiles p
    where p.id = request_row.requester_profile_id;

    insert into public.personnel_request_route_events(
      request_id,event_type,stage_label,reviewer_profile_id,reviewer_label,actor_profile_id,actor_label,detail
    ) values (
      request_row.id,'Routed',route.route_label,route.reviewer_profile_id,route.reviewer_label,null,null,'Existing open request was attached to the structured routing workflow.'
    );
  end loop;
end;
$$;
