...
alter table public.leave_requests add constraint leave_requests_leave_type_check check (leave_type = any (array['Personal'::text,'Medical'::text,'Military'::text,'Family'::text,'Paternity'::text,'Administrative'::text,'Other'::text]));
...
begin
  ...
  if p_leave_type not in ('Personal','Medical','Military','Family','Paternity','Administrative','Other') then raise exception 'Invalid leave type'; end if;
...
begin
  ...
  if leave_kind not in ('Personal','Medical','Military','Family','Paternity','Administrative','Other') then raise exception 'Invalid leave type'; end if;
