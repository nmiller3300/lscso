...
export async function POST(request: Request) {
  try {
    ...
    for (const field of requiredFields) {
      if (typeof body[field] !== "string" || !body[field].trim() || body[field].trim().length > 8000) {
        return NextResponse.json({ error: "Please answer every application question using no more than 8,000 characters." }, { status: 400 });
      }
    }
    ...
    if (!url || !key) {
      return NextResponse.json({ error: "Application service is not configured. Add SUPABASE_SECRET_KEY to the Vercel Production environment and redeploy." }, { status: 503 });
    }
    ...
    const { data: recent } = await supabase
      ...
      .ilike("discord_username", body.discord_username.trim())
