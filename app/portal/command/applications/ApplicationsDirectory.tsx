...
export function ApplicationsDirectory({ items, reviewers }: { items: Item[]; reviewers: Array<{ id: string; name: string }> }) {
  ...
  const rows = useMemo(() => items
    .filter((item) => (
      !query
      || `${item.fullName} ${item.discord} ${item.applicationTrack} ${applicationLabel(item.applicationNumber)}`.toLowerCase().includes(query.toLowerCase())
    ) && (!track || item.applicationTrack === track) && (!status || item.status === status) && (!reviewer || item.reviewerId === reviewer))
    .sort((a, b) => order === "newest"
      ? +new Date(b.submittedAt) - +new Date(a.submittedAt)
      : +new Date(a.submittedAt) - +new Date(b.submittedAt)), [items, query, track, status, reviewer, order]);
          ...
          return (
            <article key={item.id} role="listitem" className={item.closed ? "is-denied" : inInterviewWorkflow ? "is-interview" : item.status === "Submitted" ? "is-new" : item.status === "Denied" ? "is-denied" : item.hired ? "is-hired" : ""}>
              <div className="recruitment-list__identity">
                ...
                <small>{item.applicationTrack} · {item.discord}</small>
              </div>
