...
export default async function ApprovalsPage() {
  ...
  return (
    <PortalShell active="approvals" eyebrow="Command decision center" title="Approvals" description="A dedicated review queue for personnel actions that require Command authority." actions={<Link className="portal-button portal-button--secondary" href="/portal/notifications#action-required">Action Center</Link>}>
      ...
      <section className="portal-metric-grid">
        ...
        <article className="portal-metric portal-metric--neutral"><span>Certification requests</span><strong>{String(certificationRequests.length).padStart(2, "0")}</strong><small><Link href="#certification-requests">Jump to requests</Link></small></article>
      </section>
      <section className="portal-panel" id="guardian-requests"><div className="portal-panel-heading"><div><p>Guardians & personnel</p><h2>Decision queue</h2></div><span>{items.length} pending</span></div><CommandApprovalQueue initialItems={items} /></section>
      <span id="personnel-requests" className="portal-anchor-offset" aria-hidden="true" />
      <section className="portal-panel" id="leave-requests"><div className="portal-panel-heading"><div><p>Leave administration</p><h2>LOA requests</h2></div><span>{leaveItems.length} pending</span></div><LeaveApprovalQueue items={leaveItems} /></section>
      <section className="portal-panel" id="certification-requests"><div className="portal-panel-heading"><div><p>Professional standards</p><h2>Certification requests</h2></div><Link href="/portal/command/certifications">Review & issue certifications →</Link></div><div className="deputy-request-history">{certificationRequests.map((item: any) => <article key={item.id}><span>CE</span><div><strong>{item.name}</strong><small>{names.get(item.profile_id) ?? "Personnel"} · {item.notes ?? "No request note"}</small></div><b>{item.status}</b></article>)}{!certificationRequests.length ? <div className="portal-empty-state"><strong>No certification requests are awaiting review.</strong></div> : null}</div></section>
    </PortalShell>
  );
}
