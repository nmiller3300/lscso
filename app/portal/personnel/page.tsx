...
export default async function PersonnelPortalPage() {
  ...
  return (
    <PortalShell
      ...
      title={<LocalGreeting />}
      description="Your complete LSCSO service record. Rank never removes access to your own personnel information."
      actions={
        <>
          {commandAccess ? <Link className="portal-button portal-button--secondary" href="/portal/command">Command workspace</Link> : null}
          <Link className="portal-button portal-button--primary" href="#requests">Start a request</Link>
        </>
      }
    >
      <TestAccountBanner />
      ...
      <section className="deputy-profile-card">
        ...
        <div className="deputy-profile-facts">
          ...
          <div><span>Access tier</span><strong>{profile.access_tier}</strong></div>
        </div>
      ...
      <section className="portal-panel" id="discipline">
        <div className="portal-panel-heading"><div><p>Accountability</p><h2>Disciplinary standing</h2></div><span>{Number(disciplinaryPoints ?? 0)} active points</span></div>
        <div className="deputy-request-history">
          {(pointEvents ?? []).map((event) => (
            <article key={event.id}>
              <span>{event.delta > 0 ? `+${event.delta}` : event.delta}</span>
              <div><strong>{event.event_type}</strong><small>{event.reason} · {new Date(event.effective_on).toLocaleDateString()}</small></div>
              <b>{event.delta > 0 ? "Discipline" : "Restoration"}</b>
            </article>
          ))}
          {!(pointEvents ?? []).length ? <div className="portal-empty-state"><strong>No disciplinary point events are on file.</strong></div> : null}
        </div>
        <div className="deputy-record-rights"><div><strong>Restoration rules</strong><span>Outstanding Performance may restore 1 point per qualifying month. Department-head commendations may restore 1–3 points, maximum 3 per month. Your score can never fall below 0 and restoration is never banked for future discipline.</span></div></div>
      </section>

      <section className="portal-panel" id="awards">
        <div className="portal-panel-heading"><div><p>Service record</p><h2>Medals & Decorations</h2></div><span>Distinct from commendations and performance restoration</span></div>
        <div className="deputy-certification-list">
          {(awards ?? []).map((award) => (
            <article key={award.id}>
              {award.image_asset_path ? <Image src={award.image_asset_path} alt={`${award.award_name} medal`} width={54} height={54} /> : <span aria-hidden="true">★</span>}
              <div><strong>{award.award_name}</strong><small>{award.citation}</small></div>
              <div><small>Awarded</small><strong>{new Date(award.awarded_on).toLocaleDateString()}</strong></div>
              <b>Decoration</b>
            </article>
          ))}
          {!(awards ?? []).length ? <div className="portal-empty-state"><strong>No medals or decorations have been awarded.</strong><span>Approved medal artwork will display here once the LSCSO assets are uploaded.</span></div> : null}
        </div>
      </section>
      ...
      <div className="deputy-content-grid">
        ...
        <aside className="portal-panel deputy-assignments" id="assignments">
          ...
          {currentAssignments.map((assignment) => (
            <article key={assignment.id}><span>{assignment.assignment_type}</span><strong>{assignment.division}</strong><p>{assignment.notes ?? "Active assignment"}</p><small>Effective {new Date(assignment.effective_at).toLocaleDateString()}</small></article>
          ))}
          {currentAssignments.length === 0 ? <div className="portal-empty-state"><strong>No division assignments are on file.</strong></div> : null}
          <div className="deputy-acting-note"><strong>{(actingGrants ?? []).length ? "Acting-supervisor authority active" : "No active acting-supervisor authority"}</strong><span>{(actingGrants ?? []).length ? `Expires ${new Date(actingGrants![0].expires_at).toLocaleString()}` : "Temporary permissions will display here with their automatic expiration time."}</span></div>
        </aside>
      </div>

      {(flags ?? []).length ? (
        <section className="portal-panel" id="personnel-flags">
          <div className="portal-panel-heading"><div><p>Administrative indicators</p><h2>Personnel flags</h2></div><span>Flags do not replace Guardians or change disciplinary points</span></div>
          <div className="deputy-request-history">{(flags ?? []).map((flag) => <article key={flag.id}><span>PF</span><div><strong>{flag.flag_type}</strong><small>{flag.notes ?? "No administrative note"}</small></div><b>Active</b></article>)}</div>
        </section>
      ) : null}
      ...
      <section className="deputy-progress-section">
        ...
        <div className="deputy-progress-card">
          <div><span>{training?.[0]?.program_type ?? "Training record"}</span><strong>{training?.[0]?.status ?? "No active program"}</strong><p>{training?.[0]?.evaluation_notes ?? "Command has not assigned an Academy or FTO progress record."}</p></div>
          <div className="deputy-progress-track" aria-label="Training progress">{(training ?? []).map((phase, index) => <div className={["Complete", "Released"].includes(phase.status) ? "is-complete" : undefined} key={phase.id}><span>{index + 1}</span><strong>{phase.phase}</strong><small>{phase.status}</small></div>)}</div>
        </div>
      </section>
