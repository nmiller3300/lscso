...
export function PromotionCreateForm({actorRank,canInitiate,candidates}:{actorRank:string;canInitiate:boolean;candidates:PromotionCandidate[]}){
  ...
  return <>
    ...
    {open?<div className="portal-modal-backdrop" role="presentation" onPointerDown={(event)=>{if(event.currentTarget===event.target&&!pending)setOpen(false)}}><section className="portal-modal portal-modal--compact" role="dialog" aria-modal="true" aria-labelledby="promotion-create-title">
