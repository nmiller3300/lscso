...
export function PersonnelIdentityManager({ profileId, personnelId, displayName, currentRank, currentStatus }: Props) {
  ...
  return (
    <section className="portal-panel">
      <div className="portal-panel-heading"><div><p>Personnel management</p><h2>Rank & status</h2></div><span>{personnelId}</span></div>
      ...
      <div className="portal-inline-actions"><button className="portal-button portal-button--primary" disabled={!changed || pending} onClick={()=>void save()} type="button">{pending ? "Saving…" : "Apply change"}</button></div>
