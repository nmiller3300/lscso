...
type Scope = "public_site" | "personnel_portal" | "entire_system";
type Mode = "operational" | "scheduled" | "maintenance";
type StateRow = { scope: Exclude<Scope, "entire_system">; mode: Mode; effective_mode: Mode; title: string | null; public_message: string | null; internal_message: string | null; scheduled_start: string | null; expected_end: string | null; updated_at: string };
...
export function MaintenanceControlCenter() {
  ...
  async function confirm() {
    ...
    const supabase = createClient() as any;
    const { error } = await supabase.rpc("set_system_maintenance_state", {
      p_scope: pendingChange.scope,
      p_mode: pendingChange.mode,
      p_title: pendingChange.title || null,
      p_public_message: pendingChange.publicMessage || null,
      p_internal_message: pendingChange.internalMessage || null,
      p_scheduled_start: pendingChange.mode === "operational" || !pendingChange.scheduledStart ? null : new Date(pendingChange.scheduledStart).toISOString(),
      p_expected_end: pendingChange.mode === "operational" || !pendingChange.expectedEnd ? null : new Date(pendingChange.expectedEnd).toISOString(),
      p_reason: pendingChange.reason || null,
    });
    setBusy(false);
    if (error) { setNotice(error.message); return; }
    setPendingChange(null);
    setNotice(pendingChange.mode === "operational" ? "Service restored to normal operations." : "Maintenance state updated and audit event recorded.");
    setForm(emptyChange);
    await load();
  }
  ...
  return (
    <div className="maintenance-control-center">
      ...
      <section className="portal-panel maintenance-control-form">
        ...
        <form onSubmit={review}>
          <div className="portal-form-grid">
            <label>Scope<select value={form.scope} onChange={(event) => setForm({ ...form, scope: event.target.value as Scope })}><option value="personnel_portal">Personnel Portal</option><option value="public_site">Public Website</option><option value="entire_system">Entire LSCSO System</option></select></label>
      ...
      <PortalDialog open={Boolean(pendingChange)} onClose={() => { if (!busy) setPendingChange(null); }} eyebrow="Executive maintenance control" title="Confirm system state change" description="This action is recorded in the maintenance audit history and may immediately affect LSCSO users." dismissOnBackdrop={!busy} footer={<><button className="portal-button portal-button--secondary" disabled={busy} type="button" onClick={() => setPendingChange(null)}>Cancel</button><button className="portal-button portal-button--primary" disabled={busy} type="button" onClick={() => void confirm()}>{busy ? "Applying…" : "Confirm change"}</button></>}>
        {pendingChange ? <div className="maintenance-confirmation"><div><span>Scope</span><strong>{label(pendingChange.scope)}</strong></div><div><span>New state</span><strong>{label(pendingChange.mode)}</strong></div>{pendingChange.scheduledStart ? <div><span>Scheduled start</span><strong>{new Date(pendingChange.scheduledStart).toLocaleString()}</strong></div> : null}<p>{pendingChange.reason}</p></div> : null}
      </PortalDialog>
    </div>
  );
}
