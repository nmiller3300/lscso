...
export type PromotionEvaluationSummary={guardianNumber:number;kind:string;average:number;rating:string;status:string;createdAt:string;remediationRequired:boolean};
export type PromotionCaseItem={id:string;caseNumber:number;subjectPersonnelId:string;subjectName:string;currentRank:string;requestedRank:string;sourceType:string;initiatorName:string;statement:string;status:string;decisionNotes:string|null;createdAt:string;evaluation:PromotionEvaluationSummary|null;events:Array<{id:string;eventType:string;actorLabel:string|null;detail:string|null;createdAt:string}>};
...
export function PromotionCaseList({initialCases,canBegin,canDecide}:{initialCases:PromotionCaseItem[];canBegin:boolean;canDecide:boolean}){
  ...
  return <>
    <section className="portal-panel" style={{marginBottom:16}}><div className="portal-panel-heading"><div><p>Active review</p><h2>Promotion cases</h2></div><span>{open.length} open</span></div><div className="portal-approval-list">
      {open.map((item)=><article key={item.id}><span className="portal-record-type">PR</span><div className="portal-approval-id"><strong>PR-{String(item.caseNumber).padStart(4,"0")}</strong><span>{item.sourceType}</span></div><div><strong>{item.subjectName}</strong><span>{item.currentRank} → {item.requestedRank}{item.evaluation?` · ${item.evaluation.rating}`:" · No evaluation on file"}</span></div><span className="portal-priority portal-priority--routine">{item.status}</span><small>{when(item.createdAt)}</small><button className="portal-approval-review-button" onClick={()=>{setSelectedId(item.id);setNote("")}} type="button">Review →</button></article>)}
