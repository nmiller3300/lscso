...
export function ServiceRecordWorkspace({ personnel, flags, awards }: { personnel: Person[]; flags: Flag[]; awards: Award[] }) {
  ...
  async function award(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.rpc("issue_personnel_award", {
      target_profile_id: String(form.get("member")),
      award_type: String(form.get("award")),
  ...
  async function addFlag(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    const supabase = createClient() as any;
    const { error } = await supabase.rpc("add_personnel_flag", {
      target_profile_id: String(form.get("member")),
      flag_name: String(form.get("flag")),
      flag_notes: String(form.get("notes") ?? "").trim() || null,
    });
    setPending(false);
    if (error) { setNotice(error.message); return; }
  ...
  async function resolveFlag(id: string) {
    setPending(true);
    const supabase = createClient() as any;
    const { error } = await supabase.rpc("resolve_personnel_flag", { flag_id: id, resolution_notes: "Resolved through Command Service Records" });
    setPending(false);
    if (error) { setNotice(error.message); return; }
  ...
  async function restore(event: FormEvent<HTMLFormElement>) {
    ...
    const result = type === "Outstanding Performance"
      ? await supabase.rpc("award_outstanding_performance", { target_profile_id: String(form.get("member")), reason_text: String(form.get("reason") ?? "") })
      : await supabase.rpc("award_commendation_restoration", { target_profile_id: String(form.get("member")), restore_points: Number(form.get("points") ?? 1), reason_text: String(form.get("reason") ?? "") });
    setPending(false);
  ...
  return <>
    <div className="portal-dashboard-grid">
      ...
      <section className="portal-panel">
        <div className="portal-panel-heading"><div><p>Administrative indicators</p><h2>Add personnel flag</h2></div><span>Flags do not replace Guardians</span></div>
        <form onSubmit={addFlag}>
          <div className="portal-form-grid"><label>Personnel<select required name="member" defaultValue=""><option disabled value="">Select personnel</option>{personnel.map(p=><option key={p.id} value={p.id}>{p.display_name} · {p.rank}</option>)}</select></label><label>Flag<select name="flag">{flagTypes.map(f=><option key={f}>{f}</option>)}</select></label></div>
          <label className="portal-call-sign-field">Administrative notes<textarea name="notes" rows={4} /></label>
          <div className="portal-modal-actions"><button className="portal-button portal-button--primary" disabled={pending}>Add flag</button></div>
        </form>
      </section>
    </div>

    <section className="portal-panel">
      <div className="portal-panel-heading"><div><p>Disciplinary standing</p><h2>Point restoration</h2></div><span>Restoration never creates stored credit</span></div>
      <form onSubmit={restore}>
        <div className="portal-form-grid portal-form-grid--three"><label>Personnel<select required name="member" defaultValue=""><option disabled value="">Select personnel</option>{personnel.map(p=><option key={p.id} value={p.id}>{p.display_name} · {p.rank}</option>)}</select></label><label>Restoration type<select name="restorationType"><option>Outstanding Performance</option><option>Commendation Restoration</option></select></label><label>Requested restoration<select name="points"><option value="1">1 point</option><option value="2">2 points</option><option value="3">3 points</option></select></label></div>
        <label className="portal-call-sign-field">Reason<textarea required minLength={4} name="reason" rows={3} /></label>
        <div className="portal-form-protection"><strong>Enforced by Supabase</strong><span>Outstanding Performance is limited to 1 point per qualifying month. Commendation restoration is limited to 1–3 points and no more than 3 per month. Scores cannot fall below 0.</span></div>
        <div className="portal-modal-actions"><button className="portal-button portal-button--primary" disabled={pending}>Apply restoration</button></div>
      </form>
    </section>
