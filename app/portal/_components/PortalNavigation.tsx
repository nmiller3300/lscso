...
const attorneyNavigation: NavigationItem[] = [
  { id:"personnel",href:"/portal/command/personnel",label:"Personnel Directory",mobileLabel:"Personnel",glyph:"PR",section:"Legal Workspace",detail:"Department roster and personnel records" },
  { id:"personnel",href:"/portal/command/personnel/roster",label:"Department Roster",mobileLabel:"Roster",glyph:"PR",section:"Legal Workspace",detail:"Read-only department roster" },
  { id:"administration",href:"/portal/command/administration/open-records",label:"Open Records",mobileLabel:"Open Records",glyph:"AD",section:"Legal Workspace",detail:"Records-custodian review and release" },
  { id:"orders",href:"/portal/orders",label:"Command Orders",mobileLabel:"Orders",glyph:"CO",section:"Department",detail:"Department directives" },
  myInfo,
  myRequests,
];
...
export function PortalNavigation({active,audience}:PortalNavigationProps){const profile=usePortalProfile();const pathname=usePathname();const triggerRef=useRef<HTMLButtonElement>(null);const [mobileMenuOpen,setMobileMenuOpen]=useState(false);const isCommand=["Executive","Command"].includes(profile.access_tier);const isAttorney=profile.access_tier==="Attorney";const navigation=isAttorney?attorneyNavigation:isCommand?commandNavigation:["Supervisor","Preliminary"].includes(profile.access_tier)?supervisorNavigation:deputyNavigation;const sections=Array.from(new Set(navigation.map((item)=>item.section)));const workspaceLabel=isAttorney?"Legal Counsel Workspace":isCommand?"Command Operations":["Supervisor","Preliminary"].includes(profile.access_tier)?"Supervisor Workspace":"Personnel Workspace";
