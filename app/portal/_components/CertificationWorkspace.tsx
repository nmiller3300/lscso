...
type Certification = { id: string; profile_id: string; name: string; status: string; issuer: string; issued_on: string | null; expires_on: string | null; notes: string | null };
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = isCommand
      ? await supabase.rpc("issue_certification", {
          ...
          certificate_no: String(form.get("certificateNumber") ?? "").trim() || null,
      ...
      <section className="portal-panel">
        ...
        <form onSubmit={submit}>
          <div className="portal-form-grid portal-form-grid--three">
            ...
            {isCommand ? <label>Certificate number<input name="certificateNumber" placeholder="Optional internal/external ID" /></label> : <div className="portal-form-protection"><strong>Recommendation only</strong><span>This request does not add the certification until Command issues it.</span></div>}
          </div>
      ...
      <section className="portal-panel">
        ...
        <div className="deputy-certification-list">
          {certifications.map((certification) => {
            ...
            return <article key={certification.id}><span aria-hidden="true">{certification.status === "Current" ? "✓" : "…"}</span><div><strong>{certification.name}</strong><small>{person?.display_name ?? "Personnel"} · {certification.issuer}</small></div><div><small>Issued / requested</small><strong>{certification.issued_on ?? "Pending Command"}</strong></div><b>{certification.status}</b></article>;
          })}
