  ...
  return (
    <>
      <div className="deputy-request-history">
        {items.map((item) => <article key={item.id}><span>LOA</span><div><strong>{item.display_name} · {item.leave_type}</strong><small>LOA-{String(item.request_number).padStart(4, "0")} · {new Date(item.starts_on).toLocaleDateString()} → {new Date(item.expected_return_on).toLocaleDateString()}{item.notes ? ` · ${item.notes}` : ""}</small></div><div className="portal-inline-actions"><input aria-label="Review note" onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="Review note" value={notes[item.id] ?? ""} /><button disabled={pending === item.id} onClick={() => decide(item, "Approved")} type="button">Approve</button><button disabled={pending === item.id} onClick={() => decide(item, "Denied")} type="button">Deny</button></div></article>)}
